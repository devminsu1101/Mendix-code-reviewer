...
export interface ReviewIssue {
    ...
    score: number;
    /**
     * 규칙별 표에 **열**로 뜨는 짧은 사실. 값은 한두 단어여야 한다.
     * 리포트는 같은 규칙에서 나온 키들의 합집합으로 열을 만든다.
     *
     * 여기에는 관측값만 넣는다. "느릴 것 같다" 같은 추정은 넣지 않는다 —
     * 정적 분석은 실행 횟수도 데이터 규모도 모른다.
     */
    facts?: Record<string, string>;
    /**
     * **같은 규칙 안에서만** 쓰는 정렬 키. 클수록 먼저 본다.
     *
     * `score`는 `baseScore × 진입 가중치`라 한 규칙 안에서는 거의 평평하다
     * (실측: L002 9건 중 6건이 59점 동점). 그래서 규칙 안에서 무엇을 먼저 볼지
     * 가르지 못한다. 규칙마다 비용을 실제로 가르는 축이 다르므로 그 축을 여기 담는다.
     * 규칙이 다르면 이 값끼리 비교하지 않는다.
     */
    focusRank?: number;
}
