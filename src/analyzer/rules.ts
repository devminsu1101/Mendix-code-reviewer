export interface BestPractice {
    id: string;
    /**
     * 규칙을 한눈에 알아볼 짧은 이름. 리포트 섹션 제목에 ID와 함께 쓴다.
     *
     * `L003 — 10건`만 있으면 목차를 훑을 때 무슨 규칙인지 알 수 없어
     * 매번 본문을 읽어야 한다. `description`은 문장이라 제목으로 못 쓴다.
     */
    name: string;
    description: string;
    ...
    baseScore: number;
    /**
     * 이 규칙에 걸린 건들끼리 **무엇을 기준으로 먼저 보는가**. 리포트에 그대로 출력한다.
     *
     * "여기가 제일 느립니다"라고 단정하지 않기 위한 장치다. 정적 분석은 데이터 규모도
     * 호출 빈도도 모르므로 병목을 단정할 수 없다. 대신 정렬 축을 밝혀서, 읽는 사람이
     * "이건 일회성 마이그레이션이라 괜찮다" 같은 판단으로 기각할 수 있게 한다.
     */
    focusAxis?: string;
}
...
export const MendixRules: Record<string, BestPractice> = {
    ...
    LOOP_DB_RETRIEVE: {
        ...
        target: "0건",
        baseScore: 40,
        focusAxis: "반복 상한이 없는 것 먼저 → 조회 액션 수",
    },
    LOOP_COMMIT: {
        id: "L002",
        name: "루프 내 커밋",
        ...
        target: "0건",
        baseScore: 45,
        focusAxis: "반복 상한이 없는 것 먼저 → 커밋 지점 수",
    },
    HIGH_COMPLEXITY: {
        id: "L003",
        name: "로직 복잡도",
    ...
    NESTED_LOOP: {
        ...
        target: "0건",
        baseScore: 50,
        focusAxis: "중첩 깊이 → 루프 안에 DB 조회가 함께 있는 것",
    },
    ORPHAN_FLOW: {
        id: "L006",
        name: "도달 불가 flow",
    ...
    DATA_SPAGHETTI: {
        ...
        target: "5개 미만",
        baseScore: 20,
        focusAxis: "연관관계 수 → 이 엔티티를 다루는 flow 수",
    },
    ...
    MISSING_INDEX: {
        ...
        target: "조회 키 전량 커버",
        baseScore: 15,
        focusAxis: "루프 안에서 쓰이는 조회 키 → 무인/외부 경로에서 쓰이는 조회 키 → 미커버 속성 수",
    },
    NO_ACCESS_RULE: {
        id: "D004",
        name: "접근 규칙 없음",
        ...
        target: "0건",
        baseScore: 22,
        focusAxis: "이 엔티티를 다루는 flow 수 (노출 반경)",
    },
